    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#username").blur(function () {
                if ($("#username").val() == "") {
                    $("#msg22").html("<span style='color:red'>用户名不能为空</span>");
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == "error") {
                            $("#msg22").html("<span style='color:red'>已存在</span>");
                            $("#vilidatezhanghao").attr("class", "form-group has-error");
                            $("#tijiao").attr("class", "btn btn-primary disabled");
                        } else {
                            $("#msg22").html(null);
            ...
            $("#passwordagain").blur(function () {
                ...
                if ($("#passwordagain").val() != $("#password1").val()) {
        ...
        <div class="col-md-5 col-md-offset-4">
            <div class="login-panel panel panel-default">
                ...
                <div class="panel-body">
                    <form:form method="post" id="registerfrom" commandName="${commandName}" htmlEscape="true" class="form-horizontal" action="/login">
                    <form:errors path="*" id="msg" cssClass="errors" element="div" htmlEscape="false"/>
                    <div class="form-group">
                        <label for="username" class="col-sm-2 control-label">用户名</label><spring:message
                            ...
                            <c:choose>
                                <c:when test="${not empty sessionScope.openIdLocalId}">
                                    ...
                                    <input type="hidden" id="username" name="username"
                                           value="${sessionScope.openIdLocalId}" class="form-control"
                                           placeholder="请输入用户名"/>
                                </c:when>
                                <c:otherwise>
                                    <spring:message code="screen.welcome.label.netid.accesskey"
                                                    var="userNameAccessKey"/>
                                    <<%--form:--%>input class="form-control" cssClass="required" cssErrorClass="error"
                                                id="username" size="25" tabindex="1" accesskey="${userNameAccessKey}"
                                                path="username" autocomplete="off" htmlEscape="true" placeholder="请输入用户名"/>
                                </c:otherwise>
                            </c:choose>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-lg-8">
                            <<%--form:--%>input  placeholder="请输入密码" class="form-control" cssClass="required" cssErrorClass="error" id="password" size="25" tabindex="2" path="password"  accesskey="${passwordAccessKey}" htmlEscape="true" autocomplete="off"  />
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="veudyCode" class="col-sm-2 control-label">验证码</label>
                        <div class="col-lg-8">
                            <input type="text" name="veudyCode" class="form-control" id="veudyCode"
                                   placeholder="请输入验证码"/>
